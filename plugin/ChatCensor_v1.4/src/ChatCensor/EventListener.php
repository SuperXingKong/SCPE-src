		...
		if($message{0} != "/"){
			...
			if($this->plugin->isMuted(strtolower($player->getName()))){
				if($this->cfg["mute"]["log-to-player"]){
					$player->sendMessage($this->plugin->translateColors("&", Main::PREFIX . "&cYou are muted"));
				}
							...
							for($i = 0; $i < count($messagearray); $i++){
								if(in_array($messagearray[$i], $allowedchr)==false){
									if($this->cfg["char-check"]["log-to-player"] == true){
										$player->sendMessage($this->plugin->translateColors("&", Main::PREFIX . "&cEnglish standard keyboard charectars only allowed"));
									}
									$status = 1;
									$event->setCancelled(true);
								}
								...
								if($this->plugin->wordExists($messagewords[$i])){
									...
									if($tmp["sender"]["kick"] == true){
										if($iskicked == 0 && $isbanned == 0){
											$player->disconnect($tmp["kick"]["message"]);
											$iskicked = 1;
										}
									}else{
